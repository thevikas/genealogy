 ...
 * ADD CONSTRAINT `postlang_ibfk_1` FOREIGN KEY (`post_id`) REFERENCES `post` (`id`) ON DELETE CASCADE ON UPDATE
 * CASCADE;
 * </pre>
 *
 ...
 * //'localizedPrefix' => 'l_',
 * 'languages' => Yii::app()->params['translatedLanguages'], // array of your translated languages. Example : array('fr'
 * => 'Français', 'en' => 'English')
 ...
 * }
 *
 * Warning: the modification of the search criteria is based on a simple str_replace so it may not work properly under
 ...
 *
 * With this method it's possible to make multi model forms like it's explained here @link
...
class MultilingualBehavior extends CActiveRecordBehavior
{
    ...
    /**
     *
     * @var string the name of the foreign key field of the translation table related to base model table. Default to
     *      '[base model table name]_id'.
     ...
     *      Every translation for an attribute can be accessed like this: $model->[name of the attribute]_[language
     *      code] (example: $model->title_en, $model->title_fr).
     ...
     *      additionnal php file is needed.
    ...
    private function createLocalizedRelation($owner, $lang)
    {
        ...
        $owner->getMetaData ()->relations [$this->localizedRelation] = new $class ( $this->localizedRelation, $this->langClassName, $this->langForeignKey, array (
    ...
    public function attach($owner)
    {
        ...
        if (! isset ( $very_bad_global_variable_I_KNOW_doRelations ) || $very_bad_global_variable_I_KNOW_doRelations)
            $this->createLocalizedRelation ( $owner, Yii::app ()->language );
        
        $owner->getMetaData ()->relations [$this->multilangRelation] = new $class ( $this->multilangRelation, $this->langClassName, $this->langForeignKey, array (
                'index' => $this->langField 
        ) );
        ...
        if (isset ( $very_bad_global_variable_I_KNOW_dont_add_validators ) &&
                 $very_bad_global_variable_I_KNOW_dont_add_validators)
            return;
            ...
            foreach ( $this->localizedAttributes as $attr )
            {
                foreach ( $rules as $rule )
                {
                    ...
                    if (in_array ( $attr, $rule_attributes ))
                    {
                        ...
                        {
                            $validators->add ( 
                                    CValidator::createValidator ( $rule [1], $owner, $attr . '_' . $l, 
                                            array_slice ( $rule, 2 ) ) );
                        }
                        else if ($rule [1] === 'required')
                        {
                            ...
                            $validators->add ( 
                                    CValidator::createValidator ( 'safe', $owner, $attr . '_' . $l, 
                                            array_slice ( $rule, 2 ) ) );
                        }
                    }
                }
    ...
    /**
     * Dynamically create the translation model class with the use of the eval() function so no additionnal php file is
     * needed.
     * The translation model class created is a basic active record model corresponding to the translations table.
     * It include a BELONG_TO relation to the base model which allow advanced usage of the translation model like
     * conditional find on translations to retrieve base model.
     */
    public function createLangClass()
    {
        if (! class_exists ( $this->langClassName, false ))
        {
            ...
            // echo "making class with table [$this->langTableName]\n";
            eval ( 
                    "class {$this->langClassName} extends CActiveRecord
            {
     ...
     *
     * @param string $lang
     *            the lang the retrieved models will be translated (default to current application language)
     * @return model
     */
    public function localized($lang = null)
    {
        ...
        $owner->getDbCriteria ()->mergeWith ( $this->localizedCriteria () );
        return $owner;
    }
    ...
    {
        $owner = $this->getOwner ();
        $owner->getDbCriteria ()->mergeWith ( $this->multilangCriteria () );
        return $owner;
    }
    ...
    {
        $rt = array_key_exists ( $name, $this->_langAttributes );
        $mats = [ ];
        if (! $rt && preg_match ( '/(?<field>\w+)_(?<lang>\w+)/', $name, $mats ))
        {
            $field2 = $mats ['field'];
            if ($this->getOwner ()->hasRelated ( $this->multilangRelation ))
            {
                // if($mats['lang'] == $this->defaultLanguage)
                // {
                $this->setLangAttribute ( $name, empty ( $this->owner->$field2 ) ? null : $this->owner->$field2 );
        ...
        foreach ( $this->localizedAttributes as $attribute )
        {
            if (! empty ( $owner->$attribute ))
            {
                $criteria_array ['condition'] = str_replace ( $attribute . ' ', 
                        $this->localizedPrefix . $attribute . ' ', $criteria_array ['condition'] );
            }
        }
    ...
    public function afterFind($event)
    {
        ...
        {
            $this->afterFindMultilang ();
        }
        else if ($owner->hasRelated ( $this->localizedRelation ))
        {
            $this->afterFindLocalized ();
        }
    }

    public function afterFindLocalized()
    {
        $owner = $this->getOwner ();
        ...
        {
            foreach ( $this->localizedAttributes as $field )
                if (isset ( $owner->$field ) && (! empty ( $row [$this->localizedPrefix . $field] ) ||
                         $this->forceOverwrite))
                {
                    $owner->$field = $row [$this->localizedPrefix . $field];
                    
                    if (! $this->hasLangAttribute ( $langfield = $field . '_' . $this->defaultLanguage ))
                        $this->setLangAttribute ( $langfield, $row [$this->localizedPrefix . $field] );
                }
        }
    ...
    public function afterFindMultilang()
    {
        $owner = $this->getOwner ();
        ...
        foreach ( $this->languages as $lang )
            foreach ( $this->localizedAttributes as $field )
                $this->setLangAttribute ( $field . '_' . $lang, 
                        isset ( $related [$lang] [$this->localizedPrefix . $field] ) ? $related [$lang] [$this->localizedPrefix .
                                 $field] : null );
    }

    public function getLangRecord()
    {
    ...
    {
        // echo __METHOD__ . ",$name-" . parent::canGetProperty($name) . ", or " . $this->hasLangAttribute($name) .
        // "\n";
    ...
    {
        throw new Exeception ( "ding $name" );

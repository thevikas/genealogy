...
class ChartController extends Controller
{
    ...
    public function actionD3($id, $c, $jsononly = 0, $sw = 1300, $sh = 3500, $tx = 90, $ty = 0)
    {
        ...
        $this->render ( $chartname [$c], 
                [ 
                        'person' => $model,
                        'chart_code' => $c,
                        'maxlevels' => Person::$maxlevels,
                        'jsonurl' => '/chart/d3/' . $id . '?c=' . $c . '&jsononly=1',
                        'sizes' => $sizes,
                        'mother' => CHtml::link ( __ ( 'Mother' ), 
                                [ 
                                        'd3',
                                        'id' => $model->mother_cid,
                                        'c' => $c 
                                ] ),
                        'father' => CHtml::link ( __ ( 'Father' ), 
                                [ 
                                        'd3',
                                        'id' => $model->father_cid,
                                        'c' => $c 
    ...
    public function actionDownload()
    {
        ...
        if ($output_format == "svg")
        {
            ...
            // # data to the user.
            // # The only reason to use a server side script is to be able to
            // offer
